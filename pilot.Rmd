...
df$race.white <- as.numeric(grepl("1",df$Race))
df$race.lat <- as.numeric(grepl("2",df$Race))
df$race.afr <- as.numeric(grepl("3",df$Race))
df$race.asian <- as.numeric(grepl("4",df$Race))
df$race.ind <- as.numeric(grepl("5",df$Race))
df$race.isl <- as.numeric(grepl("6",df$Race))
df$race.mid <- as.numeric(grepl("7",df$Race))
df$race.other <- as.numeric(grepl("8",df$Race))
...
# Model selection - no demographics 
...
# Model selection - with demographics 

```{r}
# no feature interactions 
# with all demographics, see if any are important 

all.demo.features <- c(gender.cols, race.cols, lg.cols, relig.cols, parent.cols)
all.features <- c(all.demo.features, "platformaudience", "relationship", "location", "(1|ResponseId)")

model.formula <- as.formula(paste("rating ~ ", paste(all.features, collapse = "+")))
#model.formula
demo.model <- lmer(model.formula, data = df)
...
```{r}
# only demographic that matter is religion 
# best interactions (low AIC) is with plataud * relationship

# WHY WHEN ADDING RELIGION IN HERE, NOT STATISTIG?
# what to DO with demographics variables? 

mod <- lmer(rating ~ platformaudience*relationship + location + (1|ResponseId), data=df)
...
# I HAVE TO SEPARATE PLATFORM AND AUDIENCE? CONFUSED LOL

library(ggstance)

overall_mean <- mean(df$rating)
ems <-
  emmeans(mod, ~ platformaudience)
